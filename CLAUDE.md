...
## The real "12 Portugal itineraries": a 91-page image-only PDF, mapped and mostly transcribed (Sep 2026, unverified live)

Direct follow-up to the single-itinerary Algarve Villa PDF from the previous
entry — after being told that PDF only contained 1 of the "12 Portugal
itineraries," the DE sent a second file, `Portugal_Kensington_Itineraries_
Combined.pdf`, with no accompanying text at all. This is almost certainly
the real, complete batch originally meant by "all 12."

- **A real extraction obstacle, diagnosed properly before working around
  it.** Unlike every prior PDF this build-out has processed, `pdfminer.six`
  (this project's established fallback since `pdftoppm`/poppler-utils isn't
  installable here) returned only 91 raw form-feed characters and zero
  actual text from any page — confirmed via `pypdf.PdfReader` that the real
  page count is 91 (not the upload tool's own claimed 163, the same kind
  of wrong page-count estimate already seen and corrected once this
  session) and that every single page has a genuinely empty text layer.
  This is an **image-only PDF** — almost certainly generated by printing a
  live webpage to PDF in a way that rasterized each page rather than
  preserving real text, unlike the first (Villa) PDF, which had a clean
  text layer throughout.
- **Installed `pymupdf` (the `fitz` module) to render real page images
  since no OCR/rasterization tool was already available** — `pdftoppm`
  remains uninstallable in this environment (confirmed again, same 404 as
  every prior session), and `pdf2image` would need it too. `pymupdf` is a
  self-contained wheel with no system dependency and installed cleanly via
  `pip3 install pymupdf`. Used it to render every page as a PNG and read
  them directly via Claude's own vision — genuinely reading the document
  image-by-image, not guessing at OCR output.
- **Mapped the whole 91-page document before transcribing anything**, by
  cropping just the footer strip (URL + page-counter, e.g. "3/10") off
  every page and tiling them into 5 compact contact-sheet images — far
  cheaper to review than 91 full pages, and enough to find every itinerary
  boundary (a page counter resetting to "1/N" plus a URL change) reliably.
  This confirmed the file contains **10 real sample itineraries, not
  12** — a real, disclosed correction to what the DE's message implied,
  the same "verify the real count, don't just trust what was said" move
  already made once this session on the first PDF's page count:
  1. Best of Lisbon, Porto & the Algarve — pages 1–10
  2. Douro River Cruise: from Lisbon to Santiago de Compostela — pages 1–12
  3. Lisbon Flavors & Porto Wine — pages 1–8
  4. Perfect Pairing: Lisbon, Marrakech & Atlas Peaks — pages 1–9
  5. Portugal Golf: Lisbon & Coastal Greens — pages 1–7
  6. Portugal Golf & Culture: Lisbon to Cascais — pages 1–9
  7. Portugal Highlights: Lisbon & Porto — pages 1–8
  8. Portugal Island Gems: The Azores — pages 1–10
  9. Portugal Revealed: City to Coast — pages 1–8
  10. Spain & Portugal Signature — pages 1–10
  (10+12+8+9+7+9+8+10+8+10 = 91, exactly matching the real page count —
  confirmed the map has no gaps or overlaps before transcribing.)
- **A real, valuable independent confirmation found before writing
  anything new**: rendering each itinerary's own overview page (title,
  route, duration, starting-from price, highlights) at real resolution and
  reading it showed that **2 of these 10 already exist in the guide,
  verbatim-matching**: the Douro River Cruise (14 days, $14,146/person,
  Lisbon→Douro→Santiago de Compostela — already documented under Spain's
  own Live Website Listings section) and Spain & Portugal Signature
  (9 days, $9,999/person, Barcelona/Madrid/Lisbon — also already there).
  Both match on every figure checked — genuine confirmation that those two
  existing entries are correct, not something to duplicate or rebuild.
- **The other 8 broke down cleanly**: 5 are genuinely new content never
  documented anywhere in this guide (Lisbon Flavors & Porto Wine; Perfect
  Pairing: Lisbon, Marrakech & Atlas Peaks — a real Portugal+Morocco
  multi-country itinerary, flagged explicitly as crossing into a different
  continent, not just a Spain+Portugal pairing; Portugal Golf: Lisbon &
  Coastal Greens; Portugal Golf & Culture: Lisbon to Cascais; Portugal
  Highlights: Lisbon & Porto). The remaining 3 are the exact three
  itineraries this guide's own "PORTUGAL — TOP ITINERARIES (Job Aid)"
  section already documents (Best of Lisbon/Porto/Algarve, Portugal Island
  Gems: The Azores, Portugal Revealed: City to Coast) — those entries have
  been sitting without real pricing/links since an earlier session
  explicitly deferred that (Job-Aid-sourced content has no price/link to
  give), and this PDF is the first source to actually supply both. All
  three PDF figures (11/9/7 days) matched the Job Aid table's own "Days"
  row exactly — another clean independent confirmation, not a correction.
- **All 8 added to `KT_LIVE_ITINERARIES`** (24 entries total now, up from
  16 — re-verified via a full re-extract-and-`JSON.parse` of the array,
  confirming zero duplicate ids and every new entry well-formed) and as 8
  new `<h4>` blocks under `<h3 id="portugal-itineraries-live-website-
  listings">`, right after the existing Algarve Villa entry — same
  Route/Duration/Starting-from/Highlights/Link `<ul>` shape the Villa
  entry and Spain's own live-listing entries already use.
  **Deliberately skipped a `.media-gallery` tap-to-flip photo card for
  all 8**, same reasoning as the Villa entry: there's no verifiable-search
  method (the Wikimedia-Commons-search discipline used for real historic
  landmarks) for KT's own proprietary marketing photography, and a guessed
  CDN path can't be confirmed to resolve from this environment.
  **Deliberately skipped a full day-by-day table for all 8 too** — the
  Villa entry got one because it was a single, deep-dive addition; doing
  the same for 8 more at once would have meant transcribing ~75 more page
  images in one pass, a much larger and slower undertaking than what
  Spain's own parallel Live Website Listings section actually does (plain
  bullets, no day-by-day, for every one of its own 15 entries) — matching
  that established, lighter pattern keeps this addition consistent with
  the rest of the section rather than making these 8 an odd exception.
  The 3 that already had Job Aid content got a one-line cross-reference
  note ("the live-listing companion to the ... Job Aid entry above ...
  now with real, currently-listed pricing") rather than repeating already-
  documented client-segment/routing-logic detail a second time.
- **A real, disclosed judgment call on the footer URLs**: several page
  footers showed a trailing hyphen at the exact point the page width cut
  off (e.g. "...douro-river-cruise-from-lisbon-to-santiago-de-compostela-"),
  while others (including the itinerary with an already-fully-confirmed
  match, "best-of-lisbon-porto-and-the-algarve") had none. Since real
  website slugs essentially never end in a bare hyphen, and the
  inconsistency itself points to a print-rendering artifact rather than a
  real character, every new link was built WITHOUT the trailing hyphen —
  stated here explicitly as a judgment call, not verified against a live
  fetch (still blocked, `EGRESS_BLOCKED` for kensingtontours.com, same as
  every prior attempt this build-out).
- **The section's own intro paragraph and the Job Aid section's intro were
  both rewritten to reflect the real, now much fuller state** — the old
  "only 1 of 12 transcribed, 11 more needed" framing is replaced with an
  honest accounting of what this PDF actually contained (10, not 12),
  which 2 were already correctly documented, and that if "12" was accurate
  there are still 2 more out there somewhere, not yet sent. The Job Aid
  section now explicitly points a DE toward the live-listing section for
  actual pricing to quote from, matching how Spain's own Job-Aid/Live-
  Listings pair already cross-reference each other.
- **Sidebar nav updated** — `navsubP9` (the "Itineraries (Live website
  listings)" dropdown, previously holding just the Villa entry) now lists
  all 9 entries in this section in document order.
- Verified via this project's established non-script-content discipline:
  all 16 inline `<script>` blocks re-verified via `node --check` (all
  pass — confirms the expanded `KT_LIVE_ITINERARIES` array literal is
  syntactically valid); a full script-excluded tag-balance recount
  (div 1861/1861, table 169/169, h3 85/85, h4 244/244, and every other
  tracked tag held exactly even across the whole file); the duplicate-id
  sweep (unchanged from the established baseline — the same 4 pre-existing,
  unrelated Client Tracker bulk-action-bar ids; all 8 new itinerary ids and
  the section's own pre-existing ids confirmed genuinely unique, zero new
  duplicates); a `getElementById` cross-check (333 distinct lookups, zero
  missing targets); and the `KT_LIVE_ITINERARIES` re-extract-and-
  `JSON.parse` described above (24 total entries, zero duplicate ids).
- **What's still genuinely open, named explicitly**: if the DE's original
  "12" figure was accurate, 2 more Portugal itineraries haven't been sent
  yet — worth asking for directly rather than assuming 10 is the true
  total. None of this batch's specific facts (the exact highlight wording,
  the starting-from prices, the day-1 activity lists visible on each
  overview page) have been checked against a live source beyond the PDF's
  own rendered images — transcribed faithfully from what's legible in each
  image, not independently re-confirmed (and can't be, since
  kensingtontours.com remains unreachable from this environment). Whether
  the 8 new entries' lighter (no-photo, no-day-table) format reads as
  visually consistent with the Villa entry sitting right above them, all
  now sharing the same section, hasn't been seen in a real browser. Test
  next: open the new "Itineraries (Live Website Listings)" section and
  confirm all 9 entries render correctly and their links look plausible,
